...
Przy porównaniu przyjęto dwa zastrzeżenia obowiązujące dla wszystkich analizowanych produktów. Dostępność funkcji zależy od planu subskrypcji, konfiguracji, uprawnień i rozszerzeń, wobec czego nie założono, że każda opisana przez producenta możliwość występuje w każdej wersji. Odróżniono również deklarowaną dostępność funkcji od jej potwierdzonej skuteczności: informacja o wykorzystaniu AI nie stanowi dowodu trafności prognoz ani poprawy wyników projektowych, a niniejsza analiza nie obejmowała pomiaru jakości rekomendacji żadnego z tych narzędzi.
...
Na potrzeby niniejszej pracy Jira stanowi punkt odniesienia dla konfigurowalności procesu oraz łączenia danych projektowych z funkcjami analitycznymi.
...
Reguła automatyzacji opisuje działanie wykonywane po spełnieniu warunku i nie należy utożsamiać jej z modelem uczonym na danych ani z rozwiązaniem problemu optymalizacyjnego. Dla PlanWise istotne jest zatem oddzielenie automatyzacji obsługi zadań od analitycznej oceny ryzyka, kosztów i przydziału zasobów. Brak opisu konkretnego mechanizmu w wykorzystanym źródle nie został przy tym potraktowany jako dowód jego niedostępności w całym ekosystemie produktu.
...
Azure Boards pozwala również określać pojemność członków zespołu w sprincie, rodzaje wykonywanych działań i dni nieobecności. Rozwiązanie to pokazuje znaczenie odnoszenia dostępności do konkretnego okresu planowania i stanowi punkt odniesienia dla ograniczeń modelu pojemności przyjętego w PlanWise.\footnote{Microsoft, \textit{Set the team sprint capacity in Azure Boards}, \url{https://learn.microsoft.com/en-us/azure/devops/boards/sprints/set-capacity?view=azure-devops}, dostęp: 10.09.2026.}
...
Przypadki użycia przedstawiają działania użytkowników prowadzące do osiągnięcia określonego celu. Opisują zachowanie docelowe, a ich realizacja podlega sprawdzeniu w części testowej pracy. Dla każdego podano aktora, przebieg podstawowy oraz sytuacje, w których operacja nie dochodzi do skutku.

\begin{description}
    \item[PU-01. Utworzenie projektu i przygotowanie zespołu.]
    \label{subsec:pu-utworzenie-projektu}
    Zalogowany użytkownik podaje dane projektu i zatwierdza jego utworzenie, po czym określa uczestników wraz z ich rolami, kompetencjami, pojemnością i stawkami godzinowymi. Rezultatem jest projekt dostępny dla uprawnionych osób, zawierający dane zespołu potrzebne do dalszego planowania. Niepoprawne dane lub brak uprawnień powodują odrzucenie operacji.

    \item[PU-02. Przygotowanie backlogu i zaplanowanie sprintu.]
    \label{subsec:pu-planowanie-sprintu}
    Użytkownik uprawniony do zarządzania zadaniami tworzy je i uzupełnia opisy, oszacowania, wartość biznesową oraz zależności, następnie tworzy sprint i przypisuje do niego wybrane elementy backlogu. Po rozpoczęciu sprintu zespół aktualizuje stany zadań na tablicy. Niepoprawne daty, niedozwolone powiązania lub brak uprawnień uniemożliwiają zapis zmiany.

    \item[PU-03. Analiza ryzyka i priorytetyzacja backlogu.]
    \label{subsec:pu-ryzyko-priorytety}
    Kierownik projektu uruchamia ocenę ryzyka, a system rejestruje operację, wykonuje obliczenia i udostępnia wynik wraz z czynnikami wpływającymi na ocenę oraz nazwą metody, która ją wykonała. Następnie użytkownik uruchamia priorytetyzację i przegląda rekomendowaną kolejność, która zmienia backlog dopiero po zatwierdzeniu. Brak danych lub błąd obliczeń prowadzi do przedstawienia ograniczenia albo statusu niepowodzenia, a odrzucenie rekomendacji nie zmienia backlogu.

    \item[PU-04. Analiza harmonogramu i zastosowanie propozycji przydziału.]
    \label{subsec:pu-harmonogram}
    Użytkownik uprawniony do planowania przegląda harmonogram, terminy i zadania krytyczne, po czym uruchamia przygotowanie propozycji przydziału. System przedstawia proponowanych wykonawców wraz z przyjętą funkcją celu, ograniczeniami uwzględnionymi i pominiętymi oraz informacją o zastosowanej metodzie, a użytkownik zatwierdza całość lub wybrane zmiany. Brak odpowiednich wykonawców, cykl zależności lub zmiana danych od czasu analizy wymagają zgłoszenia problemu albo ponownego przeliczenia wyniku.

    \item[PU-05. Przygotowanie estymacji kosztów.]
    \label{subsec:pu-estymacja-kosztow}
    Użytkownik uprawniony do analizy kosztów uruchamia estymację, a system przygotowuje kontekst projektu, przekazuje go do modelu i odbiera ustrukturyzowaną odpowiedź. Po weryfikacji wynik zostaje zapisany i udostępniony wraz z wariantami, składnikami kosztów i założeniami, co pozwala porównać go z budżetem i przeanalizować rekomendacje redukcji. Błąd usługi, niezgodna struktura lub niespójne obliczenia powodują odrzucenie wyniku; niepoprawny rezultat nie jest prezentowany jako zatwierdzony kosztorys.
\end{description}
