...
Wymagania zawarte w rozdziale opisują docelowe zachowanie rozwiązania. Stopień ich realizacji wymaga odrębnej weryfikacji na podstawie implementacji i wyników testów. Rozróżnienie to jest szczególnie istotne w odniesieniu do predykcji opóźnień. Aktualna wersja projektu wyznacza oceny ryzyka za pomocą heurystyki, przy czym stanowi ona jawnie nazwany model bazowy umieszczony za wymienną granicą modelu, a system rejestruje równocześnie dane potrzebne do wytrenowania jego następcy. Samo dopasowanie modelu uczonego i porównanie go z heurystyką pozostaje natomiast poza zakresem obecnej implementacji.
...
\begin{description}
    ...
    Docelowe rozwiązanie powinno udostępniać predykcję opóźnień z wykorzystaniem modelu uczonego na zgromadzonych danych oraz umożliwiać porównanie wyników z metodą heurystyczną. Wymaganie to stanowi rozszerzenie obecnej implementacji.
    ...
    System powinien generować propozycje przydziału zadań na podstawie obciążenia, pojemności i kompetencji zespołu. Powinien wskazywać ograniczenia metody oraz umożliwiać zastosowanie całej propozycji lub wybranych zmian.
...
Ograniczenie to zostało w implementacji zaadresowane, lecz nie usunięte. Rejestrowanie wektorów cech i wyników realizacji uruchomiono, dzięki czemu zbiór uczący powstaje wraz z kolejnymi analizami. Jego liczebność zależy jednak od czasu eksploatacji systemu i liczby prowadzonych w nim projektów. W chwili sporządzania niniejszego opisu nie dysponowano zbiorem wystarczającym do dopasowania i rzetelnej oceny modelu, a ograniczenie to nie może zostać zniesione przez jednorazowe działanie implementacyjne.
