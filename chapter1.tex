...
Badanie predykcji opóźnień opiera się na danych zawierających cechy dostępne w chwili sporządzania prognozy i późniejszy wynik realizacji zadania. Ich gromadzenie zostało w systemie uruchomione, ponieważ zbioru takiego nie da się odtworzyć wstecz z danych opisujących stan bieżący. Samo dopasowanie modelu przeprowadzono natomiast na zbiorze zewnętrznym, obejmującym projekty wielu organizacji.

Podział danych ograniczał przenikanie informacji pomiędzy częścią uczącą i testową. Zastosowano dwa protokoły: podział grupowany według projektu, odpowiadający ocenie przedsięwzięć nieznanych modelowi, oraz podział według czasu. Osobno zbadano zawartość danych pod kątem wielkości opisujących przebieg realizacji, które nie mogą pełnić funkcji cech, oraz oszacowań nadanych już po rozpoczęciu sprintu. Planowane jest porównanie modelu z heurystyczną oceną ryzyka przy użyciu metryk dopasowanych do postaci problemu. W przypadku klasyfikacji obejmują one między innymi precyzję, czułość i miarę F1. Wynik regułowy nie będzie traktowany jako skalibrowane prawdopodobieństwo bez odrębnej weryfikacji.
...
Rozdział szósty koncentruje się na mechanizmach wspomagania decyzji. Opisuje ocenę ryzyka, priorytetyzację backlogu, harmonogramowanie, przydział zadań oraz estymację kosztów, wraz z ich założeniami i ograniczeniami. Osobne sekcje poświęcono pracom nad modelami: przygotowaniu wymiennych granic modeli wraz z mechanizmem gromadzenia danych uczących, doborowi zbioru danych oraz dopasowaniu, ocenie i wdrożeniu modelu predykcji opóźnień, sformułowaniu przydziału zadań jako problemu optymalizacyjnego, a także sposobowi współpracy z zewnętrznymi modelami językowymi.

Rozdział siódmy obejmuje testowanie i ocenę rozwiązania. Przedstawia wyniki wykonania zestawu testów, metodykę eksperymentów, dane i metryki, odniesienie uzyskanych wyników modelu do przyjętych kryteriów, pomocnicze studium na zbiorze odrzuconym oraz dyskusję dotyczącą weryfikacji tezy i ograniczeń badań.
